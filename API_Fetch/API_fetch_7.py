...
def api_execute(existing_file = None, initial_only=False, import_file=None, hyphen=False):
    ...
    if existing_file == None:
        ...
        start_id = scrape_results.rows[0].ID
        pickup_id = 0
    else:
        ...
        pickup_row = scrape_results.get_rows([('app_id', pickup_app_id)])[0]
        pickup_id = scrape_results.rows.index(pickup_row) + 1
        ...
        for combo in name_combos:
            ...
            try:
                if api_result[0] == "skip":
                    ...
                    skipped_filepath = project_root + '/Spreadsheets/API/' + f_prefix + 'skipped_queries' + start_time_suffix + '.xlsx'
...
def create_hyphen_subset():
    ...
    # now we create the api output file
    api_output = IM.Spreadsheet("api_output")

    for r in scrape_results.rows:
        index = scrape_results.rows.index(r)
        print("handling", str(index+1), "of", len(scrape_results.rows))
        surname = r.Surname
        print(surname, re.search(r'-', surname))
        if re.search(r'-', surname):
            clean_surname = re.sub(r'-', " ", surname)
            r.clean_surname = clean_surname
            api_output.rows.append(r)

        # if index % 1000 == 0:
        #     export_filepath = project_root + '/Spreadsheets/hypen_clean.xlsx'
        #     IM.export([api_output], export_filepath)
...
def execute_name_order_disc(disc_ids_filepath, existing_file = None):
    ...
    target_ids = IM.Spreadsheet("target_id", filepath=disc_ids_filepath, sheetname=0)
...
if __name__ == "__main__":
    ...
    existing_filename = "hyphen19341-temp.xlsx"
    existing_filepath = project_root + '/Spreadsheets/API/' + existing_filename


    api_execute(existing_file=existing_filepath, initial_only=True, import_file="hyphen_clean3.xlsx", hyphen=True)

    # excess_filename = 'initial_skipped_queies1_10000.xlsx'
    # excess_filepath = project_root + '/Spreadsheets/API/' + excess_filename

    disc_id_filename = '1-10000_nameorder_appids.xlsx'
    disc_id_filepath = project_root + '/Spreadsheets/API/' + disc_id_filename

    #existing_file
    disc_filename = '1-10000_nameorderdiscs.xlsx'
    disc_filepath = project_root + '/Spreadsheets/API/' + disc_filename

    #execute_name_order_disc(disc_ids_filepath=disc_id_filepath, existing_file=disc_filepath)

    big_filename = 'initial_skipped_queries1_10000.xlsx'
